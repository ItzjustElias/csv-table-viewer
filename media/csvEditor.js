// the main table logic, written and modified by eliasbloem
...
(function () {
  ...
  const uiState = {
    ...
    sortColumn:
      typeof persisted.sortColumn === "number" ? persisted.sortColumn : null,
  ...
  function getSortedRows(rows) {
    if (uiState.sortColumn === null || uiState.sortDirection === null)
      return rows;
    const col = uiState.sortColumn;
    const dir = uiState.sortDirection === "asc" ? 1 : -1;
    return rows
      .slice()
      .sort((a, b) => dir * compareValues(a[col] ?? "", b[col] ?? ""));
  }
  ...
  function copyToClipboard(value, td) {
  ...
  function render() {
    ...
    data.headers.forEach((headerText, index) => {
      ...
      indicator.textContent =
        uiState.sortColumn === index && uiState.sortDirection === "desc"
          ? "▼"
          : "▲";
      ...
      if (uiState.sortColumn === index) {
        th.classList.add(
          uiState.sortDirection === "asc" ? "sorted-asc" : "sorted-desc",
        );
      }
    ...
    const numericColumns = data.headers.map((_, colIndex) =>
      isNumericColumn(data.rows, colIndex, 50),
    );
    ...
    for (const row of pageRows) {
      ...
      for (let colIndex = 0; colIndex < data.headers.length; colIndex++) {
        ...
        td.title = value;
        // add a double click event listener to copy a value
        td.addEventListener("dblclick", () => copyToClipboard(value, td));
